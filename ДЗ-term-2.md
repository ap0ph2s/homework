...
---
Это механизм для корректной работы скриптов. Переопределить поведение можно с помощью флага ```ssh -t```, который принудительно выделит псевдотерминал.
...
---
Команда ```tee``` может выводить данные в файл, а так как программа запущена от имени суперпользователя имеет права на запись в ```/root/new_file```.
